        ...
        *args: list of objects
        ...
        for iteration in range(1, self.max_iter+1):
            ...
            for batch_num, batch in enumerate(dataloader, start=1):
                ...
                # NOTE: Added by Gabe Mukobi
                if batch_num % 25 == 0:
                    last_validation_score = self.validation_scores[-1] if len(self.validation_scores) > 0 else 0
                    fraction_complete_epoch = batch_num / len(dataloader)
                    fraction_complete_training = (iteration - 1 + fraction_complete_epoch) / self.max_iter
                    minutes_since_epoch_start = (time.time() - epoch_start_time) / 60
                    minutes_since_training_start = (time.time() - training_start_time) / 60
                    minutes_expected_epoch = minutes_since_epoch_start / fraction_complete_epoch * (1 - fraction_complete_epoch)
                    minutes_expected_training = minutes_since_training_start / fraction_complete_training * (1 - fraction_complete_training)
                    message = f'Epoch {iteration}/{self.max_iter} (score {round(last_validation_score, 3)}, error {round(last_epoch_error, 3)}): Batch {batch_num}/{len(dataloader)} ' \
                        f'({round(fraction_complete_epoch * 100, 2)}% epoch, {round(fraction_complete_training * 100, 2)}% training). ' \
                        f'Epoch {round(minutes_since_epoch_start, 2)} min elapsed, {round(minutes_expected_epoch, 2)} min remaining. ' \
                        f'Training {round(minutes_since_training_start, 2)} min elapsed, {round(minutes_expected_training, 2)} min remaining.'
